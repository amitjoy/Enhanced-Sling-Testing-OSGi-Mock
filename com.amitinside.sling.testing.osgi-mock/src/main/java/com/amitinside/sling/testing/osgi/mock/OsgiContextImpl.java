 ...
 * but via the OsgiContext JUnit rule.
 */
public class OsgiContextImpl {
	...
	public final <ServiceType> ServiceType getService(final Class<ServiceType> serviceType) {
		final ServiceReference serviceReference = this.bundleContext().getServiceReference(serviceType.getName());
	 ...
	 *             If the <code>filter</code> string is not a valid OSGi service
	 *             filter string.
	 */
	@SuppressWarnings("unchecked")
	public final <ServiceType> ServiceType[] getServices(final Class<ServiceType> serviceType, final String filter) {
		try {
			final ServiceReference[] serviceReferences = this.bundleContext()
					.getServiceReferences(serviceType.getName(), filter);
			if (serviceReferences != null) {
				final ServiceType[] services = (ServiceType[]) Array.newInstance(serviceType, serviceReferences.length);
				for (int i = 0; i < serviceReferences.length; i++) {
					services[i] = (ServiceType) this.bundleContext().getService(serviceReferences[i]);
				}
				return services;
			} else {
				return (ServiceType[]) new Object[0];
			}
	...
	public final <T> T registerService(final Class<T> serviceClass, final T service,
		...
		this.bundleContext().registerService(serviceClass != null ? serviceClass.getName() : null, service,
				serviceProperties);
		return service;
	}
